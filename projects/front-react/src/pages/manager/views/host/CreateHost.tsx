...
import IHostService from '../../service-api/IHostService.ts';
interface IProps {}
...
const CreateHost: React.FC<IProps> = () => {
  const navigate = useNavigate();
  const onSave = async (values: FieldType) => {
    try {
      const id = await hostService.createFile(values.name, values.description);
  ...
  return (
    <div>
      <div className="main-content__title">创建Host文件</div>
      <Form
        name="创建规则集"
        labelCol={{ span: 8 }}
        wrapperCol={{ span: 16 }}
      ...
      >
        ...
        <Form.Item<FieldType> label="文件描述" name="description" rules={[{ required: true, message: '请输文件' }]}>
          <Input.TextArea autoSize={{ minRows: 10, maxRows: 10 }} placeholder="文件描述" />
        </Form.Item>
