...
export default class ProxyConfigure extends React.PureComponent<IProps, IState> {
  ...
  formInitialValue: FieldType = getFormDataFromProfile(configureService.getConfig())
  unConfig?: () => void

  componentDidMount() {
    this.unConfig = configureService.subscribe(() => {
      const newFormValue = getFormDataFromProfile(configureService.getConfig())
  ...
  render() {
    return (
      <Form
        ...
        autoComplete="off"
      >
        <Form.Item<FieldType> label="是否开启Http代理" name="startHttpProxy" valuePropName="checked">
          <Checkbox></Checkbox>
        </Form.Item>
        <Form.Item<FieldType> label="是否开启Socks5代理" name="startSocks5" valuePropName="checked">
          <Checkbox></Checkbox>
        </Form.Item>
        <Form.Item<FieldType> label="是否开启DNS服务" name="startDns" valuePropName="checked">
          <Checkbox></Checkbox>
        </Form.Item>
        <Form.Item<FieldType> label="是否开启专业版" name="professionalVersion" valuePropName="checked">
          <Checkbox></Checkbox>
        </Form.Item>
        ...
        <Form.Item<FieldType> label="WebUi端口" name="webUiPort" rules={[{ required: true, message: '填写WebUi端口' }]}>
          <Input placeholder="WebUi端口" />
        </Form.Item>
        <Form.Item<FieldType> label="DNS端口" name="dnsPort" rules={[{ required: true, message: '填写DNS端口' }]}>
