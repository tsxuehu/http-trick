...
export default class ProfileController {
  ...
  @Path('setResolveIp', HttpMethod.POST)
  async setResolveIp(ctx: Context, next: Next) {
    let userId = ctx.userId;
    await this.profileService.setResolveIp(userId, !!ctx.query.resolve);
    ctx.body = {
      code: 0,
    };
  }
