...
export default class HostResolveService {
  ...
  async resolveIpDirect(userId: string, hostname: string, deviceId: string) {
    let result = await this.resolveIpWithWay(userId, deviceId, hostname);
    return result.ip;
  }

  async resolveIpWithoutProfile(hostname: string): Promise<string> {
    const ip = await this.dnsService.resolveIp(hostname);
    return ip;
  }

  async resolveIpAndSetInfoToContext(hostname: string, context: IProcessContext) {
    const { userId, deviceId, actualRequestData, toClientResponse } = context;
    const profile = this.profileService.getProfile(userId);
    if (profile.resolveIp) {
      toClientResponse.dnsResolveBeginTime = Date.now();
      const resolved = await this.resolveIpWithWay(userId, deviceId, hostname);
  ...
  async resolveIpWithWay(userId: string, deviceId: string, hostname: string) {
    let ip = '';
    let way = '';
