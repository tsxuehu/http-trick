...
export default class ProfileService extends EventEmitter {
  ...
  async start() {
    ...
    defaultProfile.goThroughProxyConfig = await this.fileService.readFile(
      path.resolve(appDir, 'assets/go-through-proxy-example.txt'),
    );
  ...
  generateProxyPacFile(userId: string, proxyIp: string) {
    const { startSocks5, startHttpProxy, pcIp, socks5ProxyPort, httpProxyPort } = this.appInfoService.getAppInfo();
    let proxy = 'DIRECT';
    let all = false;
    let { hostMap, globHostArray } = this._getGoThroughProxyMap(userId);
    if (hostMap['all']) {
  ...
  async setResolveIp(userId: string, enable: boolean): Promise<void> {
    let conf = this.getProfile(userId);
    conf.resolveIp = enable;
    await this.setProfile(userId, conf);
  }
